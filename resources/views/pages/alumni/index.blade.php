...
@section('content')
        ...
        <div class="pt-6 border-0 card-header">
            <div class="card-title">
                ...
                <div class="my-1 d-flex align-items-center position-relative">
                    <i class="ki-duotone ki-magnifier fs-1 position-absolute ms-4"><span class="path1"></span><span
                        ...
                        class="form-control form-control-solid w-250px ps-14" placeholder="Cari Data Alumni">
                </div>
                <!--end::Search-->
            </div>
            <div class="card-toolbar">
                <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                    ...
                    {{-- Alumni is auto-created from Mahasiswa when status = Lulus, no manual create needed --}}
                    <!--begin::Menu-->
    ...
    @includeIf('pages.alumni.modal')
    @includeIf('pages.alumni.detail')
@endsection
@push('addon-script')
    <script src="{{ URL::asset('assets/plugins/custom/datatables/datatables.bundle.js') }}"></script>
    <script>$("#kt_datepicker_1").flatpickr();</script>
    {{-- Tampil modal popup detail alumni --}}
    <script>
        $(document).on('click', '.detail-btn', function() {
            var alumniId = $(this).data('id');

            $.ajax({
                url: "{{ url('/alumni') }}/" + alumniId,
                method: 'GET',
                success: function(response) {
                    ...
                    $("#graduation_year").text(response.graduation_year);
                    $("#work_status").text(response.work_status);
                    $("#birth_place_date").text(response.birth_place + ', ' + response.birth_date);
                    $("#address").text(response.address);
                    $("#religion").text(response.religion);
                    $("#card_number_status").text(response.card_number_status);
                    $("#institution_name").text(response.institution_name);
                    $("#work_waiting_time").text(response.work_waiting_time);
                    $("#job_according_major").text(response.job_according_major);
                    $("#entry_year").text(response.entry_year);
    ...
    <script>
        ...
        var KTDatatablesExample = function() {
                ...
                datatable = $(table).DataTable({
                    ...
                    "columns": [{
                        ...
                        },
                        {
                            ...
                            className: 'no-export',
                            render: function(data, type, row) {
                                var baseUrl = "{{ route('alumni.index') }}";
            ...
            // Hook export buttons
            var exportButtons = () => {
                const documentTitle = 'Alumni Data Report';
